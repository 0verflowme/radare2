...
static void cmd_dtsra(RCore *core, const char *input) {
	if (!core || !input) {
		return;
	}
	if (!core->dbg->session) {
	...
	if (input[4] == '?') {
		r_core_cmd_help_match (core, help_msg_dts, "dtsra");
		return;
	}
	...
	bool ok = r_debug_session_restore (core->dbg, checkpoint_id);
	if (ok) {
...
static void cmd_dtsw(RCore *core, const char *input) {
	...
	if (!r_debug_session_checkpoint_replay_append (core->dbg->session, checkpoint_id, fd, bytes? bytes: (const ut8 *)"", hexlen, NULL)) {
